...
app.post("/upload_game", upload.single("file"), async (req, res) => {
  try {
    console.log("📩 Nhận request upload_game");
    ...
    if (existingFile) {
      ...
      if (
        fs.existsSync(path.join("public/upload_game", existingFile.filename))
      ) {
        fs.unlinkSync(path.join("public/upload_game", existingFile.filename));
      }
      if (fs.existsSync(path.join("public/upload_game", existingFile.name))) {
        fs.rmSync(path.join("public/upload_game", existingFile.name), {
          recursive: true,
        });
      }
    ...
    console.error("❌ Lỗi upload_game:", error);
...
app.delete("/games", async (req, res) => {
  ...
  try {
    ...
    if (!deletedGame)
      return res.status(404).send("Không tìm thấy trò chơi để xóa.");

    const { name, filename } = deletedGame;

    // Xóa trên cloud & local
    await backup_Data_Game.deleteFile(filename);
    fs.existsSync(`public/upload_game/${filename}`) &&
      fs.unlinkSync(`public/upload_game/${filename}`);
    fs.existsSync(`public/upload_game/${name}`) &&
      fs.rmSync(`public/upload_game/${name}`, { recursive: true });
...
async function checkAndDownloadFiles() {
  ...
  try {
    const filesToDownload = await File.find({
