var app = new Vue({
  ...
  computed: {
    ...
    isCanActive: function () {
      return (
        this.selectedActive !== "" && this.deactivedList[this.selectedActive]
      );
    },
  ...
  methods: {
    async loadGames() {
      ...
      const responseDeactive = await fetch(
        `/games?token=${this.token}&type=&active=false`
      );
    ...
    async updateListActive() {
      const response = await fetch(
        `/games?token=${this.token}&type=${this.category}&key=${this.key}&active=true`
      );
    ...
    async testGame() {
      ...
      const newTab = window.open(
        `/testGame?selected=${selected}&token=${this.token}`,
        "_blank"
      );
      newTab.focus();
    ...
    async hotUpdate() {
      ...
      const { name, type } = this.gamesList[this.selected];
      const response = await fetch(
        `/testGames?name=${name}&type=${type}&token=${this.token}`
      );
    ...
    async downloadGameArchive() {
      ...
      const { name, type } = this.gamesList[this.selected];
      const response = await fetch(
        `/game_archive?name=${name}&token=${this.token}`
      );
    ...
    uploadGame() {
      ...
      // Hiện loading khi bắt đầu upload
      ...
      axios(request_config)
        ...
        .finally(() => {
          // Ẩn loading sau khi hoàn tất hoặc lỗi
